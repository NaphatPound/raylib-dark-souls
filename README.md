...
Launching with no args opens a **start menu** — **free-select** any of four levels
...
| Blood-Moon Ruin | Frozen Cathedral (`ice`) |
| :---: | :---: |
| ![Blood moon](docs/screenshots/fight.png) | ![Frozen cathedral](docs/screenshots/frozen.png) |
| **Volcanic Forge** (`forge`) | **Sunken Colosseum** (`colosseum`) |
| ![Volcanic forge](docs/screenshots/forge.png) | ![Sunken colosseum](docs/screenshots/colosseum.png) |

The first three levels are `g_level` **themes** over a shared pipeline (no duplication) —
swapping lighting (`render.cpp`), sky/water shaders (`sky_*.fs` / `water_*.fs`), moon,
crystal colour + point light, and procedural props (`arena.cpp` `draw_level_props`). The
**Sunken Colosseum** goes further: it has **its own procedural geometry** (`draw_colosseum`)
— an enclosed amphitheater of concentric stepped wall tiers, four arched gateways, a
central dais and flanking braziers — and does **not** use the `boss_arena.glb` layout at
all. Concept art + build specs for the Frozen Cathedral and Sunken Colosseum were drafted
with the **Codex CLI** ([`design/`](design/)); the crescent moon's surface texture comes
from `tools/gen_moon_texture.py` (also Codex-written). The menu / level-select / save /
bonfire live in `main.cpp` + `src/save.cpp`.
